// ignore_for_file: strict_raw_type

import 'dart:async';

import 'package:flutter/cupertino.dart' show CupertinoTextField;
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

typedef OnDone = void Function(String text);
typedef OtpBoxDecoration = BoxDecoration Function(
    Color borderColor,
    Color otpBoxColor, {
    double borderWidth,
    double radius,
    });

/// class to provide standard otpBoxDecoration like standard box or underlined
class ProvidedOtpBoxDecoration {
  /// Default BoxDecoration
  static OtpBoxDecoration defaultOtpBoxDecoration = (
      Color borderColor,
      Color otpBoxColor, {
        double borderWidth = 2.0,
        double radius = 5.0,
      }) {
    return BoxDecoration(
      border: Border.all(
        color: borderColor,
        width: borderWidth,
      ),
      color: otpBoxColor,
      borderRadius: BorderRadius.circular(radius),
    );
  };

  /// Underlined BoxDecoration
  static OtpBoxDecoration underlineOtpBoxDecoration = (
      Color borderColor,
      Color otpBoxColor, {
        double borderWidth = 2.0,
        double radius = 0,
      }) {
    return BoxDecoration(
      border: Border(
        bottom: BorderSide(
          color: borderColor,
          width: borderWidth,
        ),
      ),
    );
  };

  static OtpBoxDecoration roundOtpBoxDecoration = (
      Color borderColor,
      Color otpBoxColor, {
        double borderWidth = 2.0,
        double radius = 0,
      }) {
    return BoxDecoration(
      border: Border.all(
        color: borderColor,
        width: borderWidth,
      ),
      shape: BoxShape.circle,
      color: otpBoxColor,
    );
  };
}

class ProvidedOtpBoxTextAnimation {
  ///combo of RotationTransition, DefaultTextStyleTransition, ScaleTransition
  static AnimatedSwitcherTransitionBuilder awesomeTransition =
      (Widget child, Animation<double> animation) {
    return RotationTransition(
      turns: animation,
      child: DefaultTextStyleTransition(
        style: TextStyleTween(
          begin: const TextStyle(color: Colors.pink),
          end: const TextStyle(color: Colors.blue),
        ).animate(animation),
        child: ScaleTransition(
          scale: animation,
          child: child,
        ),
      ),
    );
  };

  /// Simple Scaling Transition
  static AnimatedSwitcherTransitionBuilder scalingTransition =
      (child, animation) {
    return ScaleTransition(
      scale: animation,
      child: child,
    );
  };

  /// No transition
  static AnimatedSwitcherTransitionBuilder defaultNoTransition =
      (Widget child, Animation<double> animation) {
    return child;
  };

  /// Rotate Transition
  static AnimatedSwitcherTransitionBuilder rotateTransition =
      (Widget child, Animation<double> animation) {
    return RotationTransition(turns: animation, child: child);
  };
}

class OtpCodeTextField extends StatefulWidget {
  const OtpCodeTextField({
    super.key,
    this.isCupertino = false,
    this.maxLength = 4,
    this.controller,
    this.hideCharacter = false,
    this.highlight = false,
    this.highlightAnimation = false,
    this.highlightAnimationBeginColor = Colors.white,
    this.highlightAnimationEndColor = Colors.black,
    this.highlightAnimationDuration,
    this.highlightColor = Colors.black,
    this.otpBoxDecoration,
    this.maskCharacter = '\u25CF',
    this.otpBoxWidth = 70.0,
    this.otpBoxHeight = 70.0,
    this.otpTextStyle,
    this.onDone,
    this.defaultBorderColor = Colors.black,
    this.hasTextBorderColor = Colors.black,
    this.otpTextAnimatedSwitcherTransition,
    this.otpTextAnimatedSwitcherDuration = Duration.zero,
    this.hasError = false,
    this.errorBorderColor = Colors.red,
    this.onTextChanged,
    this.autofocus = false,
    this.focusNode,
    this.wrapAlignment = WrapAlignment.start,
    this.textDirection = TextDirection.ltr,
    this.keyboardType = TextInputType.number,
    this.otpBoxOuterPadding = const EdgeInsets.symmetric(horizontal: 4),
    this.otpBoxColor = Colors.white,
    this.highlightOtpBoxColor,
    this.otpBoxBorderWidth = 2.0,
    this.otpBoxRadius = 0,
    this.hideDefaultKeyboard = false,
    this.hasUnderline = false,
  });

  final bool isCupertino;
  final int maxLength;
  final TextEditingController? controller;
  final bool hideCharacter;
  final bool highlight;
  final bool highlightAnimation;
  final Color highlightAnimationBeginColor;
  final Color highlightAnimationEndColor;
  final Duration? highlightAnimationDuration;
  final Color highlightColor;
  final Color defaultBorderColor;
  final Color otpBoxColor;
  final Color? highlightOtpBoxColor;
  final double otpBoxBorderWidth;
  final double otpBoxRadius;
  final bool hideDefaultKeyboard;
  final OtpBoxDecoration? otpBoxDecoration;
  final String maskCharacter;
  final TextStyle? otpTextStyle;
  final double otpBoxHeight;
  final double otpBoxWidth;
  final OnDone? onDone;
  final bool hasError;
  final Color errorBorderColor;
  final Color hasTextBorderColor;
  final void Function(String)? onTextChanged;
  final bool autofocus;
  final FocusNode? focusNode;
  final AnimatedSwitcherTransitionBuilder? otpTextAnimatedSwitcherTransition;
  final Duration otpTextAnimatedSwitcherDuration;
  final WrapAlignment wrapAlignment;
  final TextDirection textDirection;
  final TextInputType keyboardType;
  final EdgeInsets otpBoxOuterPadding;
  final bool hasUnderline;

  @override
  State<StatefulWidget> createState() {
    return OtpCodeTextFieldState();
  }
}

class OtpCodeTextFieldState extends State<OtpCodeTextField>
    with SingleTickerProviderStateMixin {
  AnimationController? _highlightAnimationController;
  Animation? _highlightAnimationColorTween;
  FocusNode? focusNode;
  String text = '';
  int currentIndex = 0;
  List<String> strList = [];
  bool hasFocus = false;

  @override
  void didUpdateWidget(OtpCodeTextField oldWidget) {
    super.didUpdateWidget(oldWidget);
    focusNode = widget.focusNode ?? focusNode;

    if (oldWidget.maxLength < widget.maxLength) {
      setState(() {
        currentIndex = text.length;
      });
      widget.controller?.text = text;
      widget.controller?.selection =
          TextSelection.collapsed(offset: text.length);
    } else if (oldWidget.maxLength > widget.maxLength &&
        widget.maxLength > 0 &&
        text.isNotEmpty &&
        text.length > widget.maxLength) {
      setState(() {
        text = text.substring(0, widget.maxLength);
        currentIndex = text.length;
      });
      widget.controller?.text = text;
      widget.controller?.selection =
          TextSelection.collapsed(offset: text.length);
    }
  }

  Future<void> _calculateStrList() async {
    if (strList.length > widget.maxLength) {
      strList.length = widget.maxLength;
    }
    while (strList.length < widget.maxLength) {
      strList.add('');
    }
  }

  @override
  void initState() {
    super.initState();
    if (widget.highlightAnimation) {
      final highlightAnimationController = AnimationController(
        vsync: this,
        duration: widget.highlightAnimationDuration ??
            const Duration(milliseconds: 500),
      );
      final animationController = highlightAnimationController;

      highlightAnimationController.addStatusListener((status) {
        if (status == AnimationStatus.completed) {
          highlightAnimationController.reverse();
        } else if (status == AnimationStatus.dismissed) {
          highlightAnimationController.forward();
        }
      });
      _highlightAnimationColorTween = ColorTween(
        begin: widget.highlightAnimationBeginColor,
        end: widget.highlightAnimationEndColor,
      ).animate(animationController);
      highlightAnimationController.forward();
      _highlightAnimationController = highlightAnimationController;
    }
    focusNode = widget.focusNode ?? FocusNode();

    _initTextController();
    _calculateStrList();
    widget.controller?.addListener(_controllerListener);
    focusNode?.addListener(_focusListener);
  }

  void _controllerListener() {
    if (mounted == true) {
      setState(_initTextController);
      final onTextChanged = widget.onTextChanged;
      if (onTextChanged != null) {
        onTextChanged(widget.controller?.text ?? '');
      }
    }
  }

  void _focusListener() {
    if (mounted == true) {
      setState(() {
        hasFocus = focusNode?.hasFocus ?? false;
      });
    }
  }

  void _initTextController() {
    if (widget.controller == null) {
      return;
    }
    strList.clear();
    final text = widget.controller?.text ?? '';
    if (text.isNotEmpty) {
      if (text.length > widget.maxLength) {
        throw Exception('TextEditingController length exceeded maxLength!');
      }
    }
    for (var i = 0; i < text.length; i++) {
      strList.add(widget.hideCharacter ? widget.maskCharacter : text[i]);
    }
  }

  double get _width {
    var width = 0.0;
    for (var i = 0; i < widget.maxLength; i++) {
      width += widget.otpBoxWidth;
      if (i == 0) {
        width += widget.otpBoxOuterPadding.left;
      } else if (i + 1 == widget.maxLength) {
        width += widget.otpBoxOuterPadding.right;
      } else {
        width += widget.otpBoxOuterPadding.left;
      }
    }
    return width;
  }

  @override
  void dispose() {
    if (widget.focusNode == null) {
      // Only dispose the focus node if it's internal.  Don't dispose the passed
      // in focus node as it's owned by the parent not this child widget.
      focusNode?.dispose();
    } else {
      focusNode?.removeListener(_focusListener);
    }
    _highlightAnimationController?.dispose();
    widget.controller?.removeListener(_controllerListener);

    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
        if (!widget.isCupertino)
          _fakeTextInput()
        else
          _fakeTextInputCupertino(),
        _touchOtpBoxRow(),
      ],
    );
  }

  Widget _touchOtpBoxRow() {
    return widget.hideDefaultKeyboard
        ? _otpBoxRow(context)
        : GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () {
        if (hasFocus) {
          FocusScope.of(context).requestFocus(FocusNode());
          Future.delayed(const Duration(milliseconds: 100), () {
            FocusScope.of(context).requestFocus(focusNode);
          });
        } else {
          FocusScope.of(context).requestFocus(focusNode);
        }
      },
      child: _otpBoxRow(context),
    );
  }

  Widget _fakeTextInput() {
    const transparentBorder = OutlineInputBorder(
      borderSide: BorderSide(
        color: Colors.transparent,
        width: 0,
      ),
    );
    return SizedBox(
      width: _width,
      height: widget.otpBoxHeight,
      child: TextField(
        autofocus: !kIsWeb ? widget.autofocus : kIsWeb,
        enableInteractiveSelection: false,
        focusNode: focusNode,
        controller: widget.controller,
        keyboardType: widget.keyboardType,
        inputFormatters: widget.keyboardType == TextInputType.number
            ? <TextInputFormatter>[FilteringTextInputFormatter.digitsOnly]
            : null,
        style: const TextStyle(
          height: 0.1, color: Colors.transparent,
//          color: Colors.transparent,
        ),
        decoration: const InputDecoration(
          focusedErrorBorder: transparentBorder,
          errorBorder: transparentBorder,
          disabledBorder: transparentBorder,
          enabledBorder: transparentBorder,
          focusedBorder: transparentBorder,
          helperStyle: TextStyle(
            height: 0,
            color: Colors.transparent,
          ),
          labelStyle: TextStyle(height: 0.1),
          fillColor: Colors.transparent,
          border: InputBorder.none,
        ),
        cursorColor: Colors.transparent,
        showCursor: false,
        maxLength: widget.maxLength,
        onChanged: _onTextChanged,
      ),
    );
  }

  Widget _fakeTextInputCupertino() {
    return SizedBox(
      width: _width,
      height: widget.otpBoxHeight,
      child: CupertinoTextField(
        autofocus: !kIsWeb ? widget.autofocus : kIsWeb,
        focusNode: focusNode,
        controller: widget.controller,
        keyboardType: widget.keyboardType,
        inputFormatters: widget.keyboardType == TextInputType.number
            ? <TextInputFormatter>[FilteringTextInputFormatter.digitsOnly]
            : null,
        style: const TextStyle(
          color: Colors.transparent,
        ),
        decoration: const BoxDecoration(
          color: Colors.transparent,
        ),
        cursorColor: Colors.transparent,
        showCursor: false,
        maxLength: widget.maxLength,
        onChanged: _onTextChanged,
      ),
    );
  }

  void _onTextChanged(String text) {
    final onTextChanged = widget.onTextChanged;
    if (onTextChanged != null) {
      onTextChanged(text);
    }
    setState(() {
      this.text = text;
      if (text.length >= currentIndex) {
        for (var i = currentIndex; i < text.length; i++) {
          strList[i] = widget.hideCharacter ? widget.maskCharacter : text[i];
        }
      }
      currentIndex = text.length;
    });
    if (text.length == widget.maxLength) {
      FocusScope.of(context).requestFocus(FocusNode());
      final onDone = widget.onDone;
      if (onDone != null) {
        onDone(text);
      }
    }
  }

  Widget _otpBoxRow(BuildContext context) {
    _calculateStrList();
    final otpCodes = List<Widget>.generate(widget.maxLength, (int i) {
      return _buildOtpCode(i, context);
    });
    return Wrap(
      alignment: widget.wrapAlignment,
      textDirection: widget.textDirection,
      children: otpCodes,
    );
  }

  Widget _buildOtpCode(int i, BuildContext context) {
    Color borderColor;
    var otpBoxColor = widget.otpBoxColor;
    BoxDecoration boxDecoration;
    final highlightOtpBoxColor = widget.highlightOtpBoxColor;
    final highlightAnimationController = _highlightAnimationController;
    if (widget.hasError) {
      borderColor = widget.errorBorderColor;
    } else if (widget.highlightAnimation &&
        _shouldHighlight(i) &&
        highlightAnimationController != null) {
      return Padding(
        padding: const EdgeInsets.symmetric(horizontal: 4),
        child: AnimatedBuilder(
          animation: highlightAnimationController,
          builder: (BuildContext context, Widget? child) {
            final otpBoxDecoration = widget.otpBoxDecoration;
            if (otpBoxDecoration != null) {
              boxDecoration = otpBoxDecoration(
                _highlightAnimationColorTween?.value as Color,
                otpBoxColor,
                borderWidth: widget.otpBoxBorderWidth,
                radius: widget.otpBoxRadius,
              );
            } else {
              boxDecoration = ProvidedOtpBoxDecoration.defaultOtpBoxDecoration(
                _highlightAnimationColorTween?.value as Color,
                otpBoxColor,
                borderWidth: widget.otpBoxBorderWidth,
                radius: widget.otpBoxRadius,
              );
            }

            return Container(
              key: ValueKey<String>('container$i'),
              decoration: boxDecoration,
              width: widget.otpBoxWidth,
              height: widget.otpBoxHeight,
              child: Center(child: _animatedTextBox(strList[i], i)),
            );
          },
        ),
      );
    } else if (widget.highlight && _shouldHighlight(i)) {
      borderColor = widget.highlightColor;
      if (highlightOtpBoxColor != null) {
        otpBoxColor = highlightOtpBoxColor;
      }
    } else if (i < text.length) {
      borderColor = widget.hasTextBorderColor;
      if (highlightOtpBoxColor != null) {
        otpBoxColor = highlightOtpBoxColor;
      }
    } else {
      borderColor = widget.defaultBorderColor;
      otpBoxColor = widget.otpBoxColor;
    }
    final otpBoxDecoration = widget.otpBoxDecoration;
    if (otpBoxDecoration != null) {
      boxDecoration = otpBoxDecoration(
        borderColor,
        otpBoxColor,
        borderWidth: widget.otpBoxBorderWidth,
        radius: widget.otpBoxRadius,
      );
    } else {
      boxDecoration = ProvidedOtpBoxDecoration.defaultOtpBoxDecoration(
        borderColor,
        otpBoxColor,
        borderWidth: widget.otpBoxBorderWidth,
        radius: widget.otpBoxRadius,
      );
    }
    EdgeInsets insets;
    if (i == 0) {
      insets = EdgeInsets.only(
        top: widget.otpBoxOuterPadding.top,
        right: widget.otpBoxOuterPadding.right,
        bottom: widget.otpBoxOuterPadding.bottom,
      );
    } else if (i == strList.length - 1) {
      insets = EdgeInsets.only(
        left: widget.otpBoxOuterPadding.left,
        top: widget.otpBoxOuterPadding.top,
        bottom: widget.otpBoxOuterPadding.bottom,
      );
    } else {
      insets = widget.otpBoxOuterPadding;
    }
    return Padding(
      padding: insets,
      child: Container(
        key: ValueKey<String>('container$i'),
        decoration: boxDecoration,
        width: widget.otpBoxWidth,
        height: widget.otpBoxHeight,
        child: Padding(
          padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 8),
          child: DecoratedBox(
            decoration: BoxDecoration(
              border: widget.hasUnderline
                  ? Border(
                bottom: BorderSide(
                  color: borderColor,
                ),
              )
                  : null,
            ),
            child: Center(child: _animatedTextBox(strList[i], i)),
          ),
        ),
      ),
    );
  }

  bool _shouldHighlight(int i) {
    return hasFocus &&
        (i == text.length ||
            (i == text.length - 1 && text.length == widget.maxLength));
  }

  Widget _animatedTextBox(String text, int i) {
    if (widget.otpTextAnimatedSwitcherTransition != null) {
      return AnimatedSwitcher(
        duration: widget.otpTextAnimatedSwitcherDuration,
        transitionBuilder: widget.otpTextAnimatedSwitcherTransition ??
                (Widget child, Animation<double> animation) {
              return child;
            },
        child: Text(
          text,
          key: ValueKey<String>('$text$i'),
          style: widget.otpTextStyle,
        ),
      );
    } else {
      return Text(
        text,
        key: ValueKey<String>('${strList[i]}$i'),
        style: widget.otpTextStyle,
      );
    }
  }
}